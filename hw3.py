...
def input(line):
    ...
    if len(line) >= 1:
        ...
        if words[0] == "sun":
            lights.append(
                [float(words[1]), float(words[2]), float(words[3]), color, "sun"]
            )
        if words[0] == "color":
            color = [
                int(255 * float(words[1])),
                int(255 * float(words[2])),
                int(255 * float(words[3])),
                255,
            ]
        if words[0] == "bulb":
            bulb_count += 1
...
def raytrace():
    ...
    for x in range(0, width):
        for y in range(0, height):
            ...
            sx, sy = (
                round((2 * x - width) / (max(width, height)), 5),
                round((height - 2 * y) / max(width, height), 5),
            )
            r_sq = 0
            if fisheye:
                sx /= vec_mag(forward)
                sy /= vec_mag(forward)
                r_sq = sx ** 2 + sy ** 2
                if round(r_sq ** 0.5, 5) > 1:
                    continue
                else:
                    forward = normalize(forward)
                    forward = scal_mult((1 - r_sq) ** 0.5, forward)
            ...
            if closest != sys.maxsize:
                ...
                surface_point = [
                    eye[0] + ray_dir[0] * closest,
                    eye[1] + ray_dir[1] * closest,
                    eye[2] + ray_dir[2] * closest,
                ]
                obj_color = []
                ...
                if closest_obj[0][:6] == "Sphere":
                    normal = normalize(
                        [
                            surface_point[0] - closest_obj[1],
                            surface_point[1] - closest_obj[2],
                            surface_point[2] - closest_obj[3],
                        ]
                    )
                    ...
                    _BA, _CA, _CB = (
                        vec_add([closest_obj[2], scal_mult(-1, closest_obj[1])]),
                        vec_add([closest_obj[3], scal_mult(-1, closest_obj[1])]),
                        vec_add([closest_obj[3], scal_mult(-1, closest_obj[2])]),
                    )
                ...
                for light in lights:
                    ...
                    if light[-1][:4] == "bulb":
                        ...
                        l_dir = [
                            -1 * surface_point[0] + light[0],
                            -1 * surface_point[1] + light[1],
                            -1 * surface_point[2] + light[2],
                        ]
                        dist = vec_mag(l_dir)
                    ...
                    if (
                        vec_dot(
                            normal,
                            [
                                surface_point[0] - eye[0],
                                surface_point[1] - eye[1],
                                surface_point[2] - eye[2],
                            ],
                        )
                        > 0
                    ):
                    ...
                    l_col = elem_mult(
                        scal_mult(1 / 255, light[3]), scal_mult(1 / 255, closest_obj[5])
                    )
...
def Ray_Sphere_Int(
    sphere, ray_dir, origin
):  # checks to see if ray intersects object, returns t value.
    inside = (
        True
        if vec_mag(
            [sphere[1] - origin[0], sphere[2] - origin[1], sphere[3] - origin[2]]
        )
        ** 2
        < sphere[4] ** 2
        else False
    ...
    square_dis = round(
        vec_mag(vec_add([ray, [-1 * sphere[1], -1 * sphere[2], -1 * sphere[3]]])) ** 2,
        5,
    )
    ...
    t_off = round((sphere[4] ** 2 - square_dis) ** 0.5, 5)
    if inside:
        return t_c + t_off
    return t_c - t_off
...
def Ray_Tri_Int(tri, ray_dir, origin):
    _BA, _CA, _CB = (
        vec_add([tri[2], scal_mult(-1, tri[1])]),
        vec_add([tri[3], scal_mult(-1, tri[1])]),
        vec_add([tri[3], scal_mult(-1, tri[2])]),
    )
